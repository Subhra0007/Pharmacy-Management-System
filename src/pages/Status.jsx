...
export default function Status() {
      ...
      const { columns, employees } = statusData;

      const getEmployeesByStatus = (statusId) => {
            return employees.filter((emp) => emp.status === statusId);
      };
                        ...
                        {columns.map((col) => {
                              ...
                              return (
                                    <div key={col.id} className="min-w-[320px] flex-1">
                                          ...
                                          <div className={`p-4 rounded-b-xl min-h-[500px] ${darkMode ? "bg-gray-800" : "bg-gray-100"}`}>
                                                <div className="space-y-4">
                                                      {colEmployees.map((emp) => (
                                                            <div key={emp.id} className={`p-4 rounded-lg shadow-sm border transition-shadow hover:shadow-md ${darkMode ? "bg-gray-700 border-gray-600" : "bg-white border-gray-200"}`}>
